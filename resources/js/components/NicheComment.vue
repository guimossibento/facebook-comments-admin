          ...
          <div class="card" v-if="$gate.isAdmin()">
            <div class="card-header text-center">
              <div class="row-cols-1">
                <h3 class="card-title float-left">Vínculo Nicho X Comentários</h3>
              </div>
            </div>
            <!-- /.card-header -->
  ...
  methods: {
    onChangeList: function (value, direction, movedKeys) {
      ...
      value.forEach((element) => {
        comments.data.push({ id: element });
      });

      axios
          .put(`api/niches/${this.form.niche}/comments`, comments)
          .then((response) => {})
          .catch((error) => {
            Toast.fire({
              icon: "error",
    ...
    loadComments(niche) {
      this.$Progress.start();
      axios
          .get(`api/niches/${niche}?include=comments`)
          .then(({ data }) => {
            data = data.data["comments"].map(function (comment) {
              return comment.id;
            });
      ...
      axios.get(`comments/list`).then(({ data }) => {
